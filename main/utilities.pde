...
void cwf(color c, int w) {stroke(c); strokeWeight(w); fill(c); } // color, width, fill
...
void displayHeader()  // Displays title and authors face on screen
    ...
    float w1 = PictureOfStudent1.width, h1 = PictureOfStudent1.height;
    float s1 = min(75./w1,100/h1);
    w1*=s1; h1*=s1;
    image(PictureOfStudent1, width-w1,30,w1,h1); 
    if(team)
      {
      float w2 = PictureOfStudent1.width, h2 = PictureOfStudent1.height;
      float s2 = min(75./w2,100/h2);
      w2*=s2; h2*=s2;
      image(PictureOfStudent2, width-w2-w1,30,w2,h2); 
      }
    }
