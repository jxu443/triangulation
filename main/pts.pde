...
class PNTS 
 ...
 // CREATE
  ...
  void resetRandomPoints() {resetRandomPoints(pointCount); } 
  
  void resetRandomPoints(int k)                                                         // init the points to be on a well framed circle
    {
    int mx=30, my=150;
    empty();
    for (int i=0; i<k; i++)
      addPoint(random(mx,width-mx),random(my,height-my));
    } 
  ...
  void drawPolyline() 
    ...
    void drawPolylineWithGaps() 
    {
    beginShape(); 
      for (int v=0; v<pointCount; v++) 
        {
        if(d(G[max(0,v-1)],G[v])>25) {endShape(); beginShape();}
        G[v].v(); 
        }
    endShape(); 
    }
