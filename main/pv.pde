...
void label(float x, float y, String S, color c) {fill(c); text(S,x,y); noFill();}                                                         // writes string S next to P on the screen ( for example label(P[i],str(i));)
...
void show(PNT P, VCT V, String S) {show(P,V); P(P(P,0.70,V),-17,R(U(V))).label(S);}       // show V as arrow from P and print string S on its side
